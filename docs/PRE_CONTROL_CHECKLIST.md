...
- [ ] Gate G2: the 2026-09-26 e5486d5 physical retry failed safely before cycle 1 READY and before any fan write. Baseline/service state was clean (FF/FF, journal absent, LocalSystem watchdog Ready). The watchdog authenticated GUI PID 34624, proving the local backend admission EC check had completed and the named-pipe Hello succeeded, then admission failed before PREPARED/OWNED with the old generic FanControlAdmissionException; final EC was independently FF/FF, journal absent and the emergency fallback did not execute. Source audit localized the remaining failure window to watchdog Prepare. Its service-side ReadSetpointAsync still opened Hp88F8EcControlStateProbe.Read(), taking a full 88F8 EC snapshot (rates/manual/mode/tachs as well as ownership) under Global\\Access_EC even though Prepare needs only 0x34/0x35. That unnecessarily enlarged cross-process EC contention against live GUI telemetry/control. The watchdog lease hardware now uses a narrow single-mutex 0x34/0x35 setpoint snapshot only, and backend no-write admission exceptions preserve the exact inner exception type/message so a future failure is causally diagnosable rather than generic. Gate G2 inter-cycle stabilization from the prior fix remains in place. Re-run 5/5 and require the same watchdog/GUI identities, exact OWNED 30/30, real S3 causality, prompt suspend fencing, completed Firmware + FF/FF + watchdog Release/Ready + journal absent before resume acceptance, exactly one accepted resume, five-snapshot Healthy recovery, one controlled re-entry, bounded two-snapshot Firmware-only inter-cycle stabilization, no watchdog recovery/fallback execution, and unchanged OGH undervolt.
