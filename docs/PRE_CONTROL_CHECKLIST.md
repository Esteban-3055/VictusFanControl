...
- [ ] Gate G2: run 5/5 consecutive lifecycle cycles under the same revised contract without intentionally restarting the GUI or watchdog service between cycles. Physical attempt on 2026-09-26 passed cycle 1 completely under watchdog PID 16564 / GUI PID 7308 (OWNED 30/30, real S3, preBlock 199.9 ms, restore 1173.4 ms, Firmware 1173.8 ms, exactly one resume, Healthy recovery, controlled re-entry, final Firmware + FF/FF + journal absent) but cycle 2 failed safely before READY: Custom admission briefly succeeded and the normal telemetry safety supervisor immediately handed authority back to Firmware before the 30/30 command, producing 'Fan command refused because custom authority is not active'. Final EC was independently FF/FF, journal absent and fallback did not execute. The cause is an inter-cycle orchestration race: cycle 2 was armed immediately after the previous real re-entry/restore while the independent telemetry stream could still publish a transient post-control snapshot. The fix does not relax SafetyGate; between cycles the GUI now remains Firmware-only and requires two distinct complete, Healthy, SafetyGate-permitted, light-load snapshots captured after the previous Firmware transition before reacquiring Custom. The wait is bounded to 15 s and any genuinely unsafe/incomplete stream still fails closed. Re-run 5/5 and require the same watchdog PID across all cycles, exact durable OWNED 30/30 before each suspend, prompt pre-block fencing/Telemetry Suspended, completed Firmware + backend FF/FF + watchdog Release/Ready + journal absent before each resume is accepted, exactly one accepted resume per cycle, five-snapshot Healthy recovery, one controlled 30/30 re-entry, final Firmware + FF/FF + journal absent, no timeout/recovery/fatal evidence, no emergency fallback execution, and OGH undervolt unchanged.
