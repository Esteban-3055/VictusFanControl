...
- [~] Gate D implementation is complete and synthetic/CI regressions are green; physical validation is pending. The persistent LocalSystem service hosts the durable lease, restore-only HP hardware authority, protected local IPC, exact PID+creation-time identity binding on every lease mutation, independent controller-process monitoring and state deadlines. The HP backend durably acknowledges WriteIntent before WMI dispatch, re-checks EC ownership after the IPC/flush window, Commit occurs only after EC+dual-tach ACK, healthy status drives heartbeat, and pre-WMI rollback preserves external ownership. A live-controller pipe break now retains the lease and allows only same-identity reconnect instead of restoring underneath a still-running controller. The physical gate must still prove OWNED 30/30 -> exact GUI forced kill -> service-only `FF,FF -> LegacyDefault` -> journal deletion -> independent FF/FF, with no parent-shell restore.
