...
- [x] Gate G1 revised-contract regression passed on physical S3 hardware on 2026-09-26 at commit 626f695: watchdog PID 25624 and GUI PID 2112 reached exact durable OWNED generation 3 at 30/30; PBT_APMSUSPEND fenced admission and marked telemetry Suspended in 234.6 ms; validated restore completed with backend FF/FF acknowledgement and watchdog Release (restore 1009.5 ms, Firmware transition 1009.9 ms) before resume acceptance; the same watchdog PID was Ready with journal absent; exactly one resume was accepted; five-snapshot recovery reached Healthy; one controlled post-resume 30/30 re-entry succeeded; final authority was Firmware with EC FF/FF and journal absent. Kernel-Power 42 -> 107 confirmed real S3, the emergency fallback did not execute, no watchdog timeout/recovery/fatal evidence occurred, and OGH undervolt was SAME.
- [ ] Gate G2: latest physical retry on 2026-09-26 at 1bd2d79 reached cycle 1 READY with durable OWNED 30/30, completed real S3 handoff (preBlock 208.2 ms, local restore 722.0 ms, Firmware 722.4 ms), exactly one accepted resume, five-snapshot Healthy recovery and a controlled post-resume 30/30 re-entry. The final local restore then reached Firmware + EC FF/FF, but the durable watchdog journal was still PRESENT, so the cycle correctly failed instead of claiming PASS. The watchdog pipe closed immediately afterward while the GUI was still alive; after GUI exit, owner-loss recovery observed FF/FF, normalized the still-RESTORING lease and cleared the journal. Source audit found a structural false-negative in watchdog recovery: unlike Gate B and the production backend, WatchdogLeaseManager issued HP RestoreFirmwareAuto and trusted one immediate EC read. HP/WMI completion and EC publication are not guaranteed to be synchronous, so a legitimate short-lived old setpoint/read transient can retain RESTORING even though FF/FF appears moments later. Watchdog recovery now polls the narrow 0x34/0x35 setpoint for up to 5 s at 250 ms using the sleep-excluding watchdog clock; it deletes the journal only after verified FF/FF, immediately stops fail-closed on any unexpected external fixed setpoint, and retains the journal on timeout/read failure. A synthetic Gate C regression delays FF/FF publication and requires Release to wait and then clear the lease. Gate G final re-entry now also requires backend WatchdogReleaseVerified in addition to Firmware + FF/FF + journal absence, and the pipe server logs explicit Release ACK/rejection detail. Re-run 5/5 with the same watchdog/GUI identities, exact OWNED 30/30, real S3 causality, prompt suspend fencing, completed Firmware + FF/FF + watchdog Release/Ready + journal absent before resume acceptance, exactly one accepted resume, five-snapshot Healthy recovery, one controlled re-entry, bounded two-snapshot Firmware-only inter-cycle stabilization, no watchdog recovery/fallback execution, and unchanged OGH undervolt.
- [ ] Before unattended Custom/adaptive control, characterize an S3-interrupted restore from the lowest intended Custom target or implement a validated suspend-safety floor/elevation. Gate G currently uses 30/30, so allowing the durable restore transaction to span S3 does not by itself validate wake behavior from future low targets such as level 14.
