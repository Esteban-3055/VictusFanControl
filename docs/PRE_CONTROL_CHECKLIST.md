...
- [ ] Gate G2: repeat the complete Gate G1 lifecycle for 5/5 consecutive cycles without intentionally restarting the GUI or watchdog service between cycles; require one accepted resume per cycle, duplicate resumes coalesced, same watchdog PID, no stale re-admission, no journal leak, and final Firmware + FF/FF + journal absent. Physical attempt 1 passed cycle 1 but failed cycle 2 because a redundant post-restore EC proof crossed S3. Physical attempt 2 then proved the stronger rule: the coordinator restore and TelemetryWorker Suspended transition completed before sleep, but the following read-only watchdog status/journal proof was frozen by S3 and completed only after resume (marker: telemetry=Recovering, acceptedResumesBeforeProof=1, handlerMs about 28.5 s). Both runs failed closed and ended with independently proven journal absence + EC FF/FF; fallback did not execute. Current fix removes all post-restore filesystem/EC/WMI/watchdog IPC from PBT_APMSUSPEND, captures backend local-FF/FF + successful-watchdog-Release evidence in memory within the 1800 ms budget, and persists the marker only after resume. A new 5/5 hardware run is still required.
