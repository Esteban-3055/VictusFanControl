...
- [x] Gate G1 passed on physical S3 hardware on 2026-09-25: watchdog PID 29972 and GUI PID 26944 reached exact durable OWNED generation 3 at 30/30; PBT_APMSUSPEND completed the normal RestoreBegin -> local FF/FF -> LegacyDefault -> Release handoff before return; the durable pre-sleep marker proved Firmware + EC 255/255 + journal absent under the same watchdog PID. Kernel-Power 42 -> 107 confirmed a real Application API suspend/resume. After resume, telemetry recovered to Healthy, the watchdog remained PID 29972/Ready with journal absent, one controlled 30/30 re-entry succeeded, and the final state was Firmware + FF/FF + journal absent. No OWNED/WRITE_ARMED/RESTORING timeout, OwnershipAmbiguous, fatal/recovery path or fallback execution occurred; OGH undervolt was SAME.
- [ ] Gate G2: repeat the complete Gate G1 lifecycle for 5/5 consecutive cycles without intentionally restarting the GUI or watchdog service between cycles; require one accepted resume per cycle, duplicate resumes coalesced, same watchdog PID, no stale re-admission, no journal leak, and final Firmware + FF/FF + journal absent. Physical attempt 1 passed cycle 1 but failed cycle 2 because a redundant post-restore EC proof crossed S3. Attempt 2 showed the subsequent watchdog status/journal read could also be frozen by S3. A one-cycle G1 regression of the deferred-marker fix then showed an even narrower race: the backend restore and Firmware transition completed before sleep, but Windows froze the UI thread before the caller finally block could create the in-memory marker. All failures were fail-closed and ended with independently proven journal absence + EC FF/FF; fallback did not execute. Current fix no longer depends on any caller continuation after restore: admission is fenced and telemetry is synchronously marked Suspended before restore IO, backend local-FF/FF + watchdog-Release evidence and coordinator Firmware timestamp are captured inside their owning components, and the durable pre-sleep marker is reconstructed/persisted after resume only if all causal timestamps precede resume and complete within 1800 ms. Re-pass one-cycle G1 before retrying G2 5/5.
