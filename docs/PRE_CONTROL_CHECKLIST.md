...
- [ ] Gate G2: repeat the complete Gate G1 lifecycle for 5/5 consecutive cycles without intentionally restarting the GUI or watchdog service between cycles; require one accepted resume per cycle, duplicate resumes coalesced, same watchdog PID, no stale re-admission, no journal leak, and final Firmware + FF/FF + journal absent. First physical attempt on 2026-09-25 passed cycle 1 but failed cycle 2 because the test-only post-restore full EC proof overran the PBT_APMSUSPEND window: resume was accepted before the pre-sleep marker/NotifySuspend sequence completed. The run failed closed, the GUI was killed, journal was absent, final EC was independently FF/FF and fallback did not run. Fix: remove that redundant Gate-G EC read, mark telemetry Suspended immediately after the coordinator restore, use the production backend return as FF/FF proof, reject any resume-before-proof, and enforce a 1800 ms proof budget before rerunning 5/5.
