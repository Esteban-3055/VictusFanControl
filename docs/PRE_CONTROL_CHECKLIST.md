...
- [ ] Gate G1 revised-contract regression + Gate G2 5/5: later physical runs exposed three distinct PBT_APMSUSPEND races (redundant EC proof crossing S3; watchdog status/journal proof crossing S3; caller finally/marker continuation crossing S3). A later regression showed TelemetryWorker reached Suspended in 266.7 ms but the validated restore itself completed only after wake (restore evidence 31016.5 ms, Firmware 31016.9 ms), leading to the resumable durable-lease contract. A follow-up source audit then found two more sleep-counting timeout domains (HP backend Stopwatch/QPC ACK waits and .NET 8 named-pipe CancelAfter), both now replaced with QueryUnbiasedInterruptTime-derived active time and covered by synthetic frozen-clock regressions. The next physical attempt on 2026-09-26 failed safely before READY, before any 30/30 write or S3: baseline watchdog/FF state was clean, the service never progressed beyond waiting for its first controller connection, and final EC remained FF/FF. Source review isolated a no-write SafetyGate sequence race in which the 1 Hz telemetry supervisor can supersede Gate G's healthy admission evaluation before TryEnterCustomAsync. Gate G now retries initial admission, post-resume re-entry and pre-dispatch command safety only when the attempted evaluation is proven superseded; a current denial remains fail-closed, and command retries require the typed pre-dispatch stale-safety exception while Custom authority is still intact. Re-pass one-cycle G1 under this complete timeout + safety-ordering model, then run 5/5 consecutive cycles without intentional GUI/service restart, with one accepted resume per cycle, duplicate resumes coalesced, same watchdog PID, no stale re-admission, no journal leak, and final Firmware + FF/FF + journal absent.
