...
- [~] Independent crash watchdog/lease: Gate A read-only Windows Service environment is implemented; physical Session 0 validation is pending. Gate A tests exact fingerprint, PawnIO EC reads and HP WMI GetFanLevel under LocalService first, with LocalSystem only as a controlled fallback. No fan writes exist in Gate A.
