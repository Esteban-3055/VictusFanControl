...
- [~] Gate F is staged as F1 + F2. **F1 passed on real hardware on 2026-09-25**: production watchdog PID 10508 and GUI PID 16076 reached durable OWNED generation 3 at 30/30 after backend EC+dual-tach ACK; the harness killed the watchdog first and issued the GUI kill only 1.071 ms later, both original processes were confirmed dead, no live-GUI restore-start marker appeared, and SCM replacement PID 13288 recovered the retained journal with `RestoredFirmware` and returned EC to FF/FF. The emergency fallback remained unused, production watchdog PID 27172 returned Ready with recovery re-verified at 1 s / 5 s / 10 s, and OGH undervolt remained unchanged. F2 is implemented and awaiting physical validation: a test-only lease wrapper holds the real controller inside `CommitAsync` after WMI + EC-setpoint + dual-tach acknowledgement but before Commit is forwarded, while the journal remains first-write WRITE_ARMED pending 30/30. Its harness requires the same bounded watchdog-first double kill, exact identities, no GUI restore-start, replacement-watchdog `RestoredFirmware`, journal clear, FF/FF, unused fallback and unchanged OGH undervolt. Gate F remains open until F2 passes.
