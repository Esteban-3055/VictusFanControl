...
- [ ] Gate G2: run 5/5 consecutive lifecycle cycles under the same revised contract without intentionally restarting the GUI or watchdog service between cycles. Require the same watchdog PID across all cycles, exact durable OWNED 30/30 before each suspend, prompt pre-block fencing/Telemetry Suspended, completed Firmware + backend FF/FF + watchdog Release/Ready + journal absent before each resume is accepted, exactly one accepted resume per cycle, five-snapshot Healthy recovery, one controlled 30/30 re-entry, final Firmware + FF/FF + journal absent, no stale re-admission, no timeout/recovery/fatal evidence, no emergency fallback execution, and OGH undervolt unchanged.
