...
- [~] Gate D implementation is complete and synthetic/CI regressions are green; physical validation is pending. The persistent LocalSystem service now hosts the durable lease, real controller PID/process-lifetime monitoring, protected Administrator-only local IPC, deadline supervision and restore-only HP hardware authority. The HP backend durably acknowledges WriteIntent before WMI dispatch, re-checks EC ownership after the IPC/flush window, Commit occurs only after EC+dual-tach ACK, healthy status drives heartbeat, and pre-WMI rollback preserves external ownership. The physical gate must still prove OWNED 30/30 -> exact GUI forced kill -> service-only `FF,FF -> LegacyDefault` -> journal deletion -> independent FF/FF, with no parent-shell restore.
