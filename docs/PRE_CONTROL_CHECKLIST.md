...
- [~] Gate F is staged as F1 + F2. F1 (OWNED double death) is implemented and awaiting physical validation: after production backend EC+dual-tach ACK and durable OWNED 30/30, the harness validates exact GUI/watchdog identities, force-kills watchdog then GUI within a measured <=50 ms issue bound, rejects any live-GUI restore-start marker, and requires the production SCM replacement watchdog to recover the retained journal with `RestoredFirmware`, clear it only after verified FF/FF, leave the emergency fallback unused, restore the production service baseline and preserve OGH undervolt. F2 will validate the WRITE_ARMED post-WMI/post-ACK but pre-Commit window. Gate F remains open until both subgates pass.
