...
- [~] Watchdog Gate B service-only restore primitive implemented with synthetic coverage and an ownership-safe delayed fallback; real-hardware validation is pending. The one-shot service accepts only an explicitly armed 30/30 test state and can only execute `FF,FF -> LegacyDefault -> verify EC FF/FF`.
