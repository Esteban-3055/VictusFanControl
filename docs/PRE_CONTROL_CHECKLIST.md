...
- [~] Forced-process-termination / EC countdown-watchdog harness implemented (`scripts/test-forced-kill-watchdog.ps1`); real-target characterization still required. The harness observes 0x34/0x35 and 0x62/0x63 read-only, force-kills only the VFC GUI after verified 30/30, and arms an independent delayed FF/FF -> LegacyDefault cleanup before the kill.
