...
function Assert-ProductionServiceReady {
    ...
    if ($ExpectedPid -gt 0 -and $pid -ne $ExpectedPid) {
        throw "Watchdog PID changed during Gate G1: $ExpectedPid -> $pid."
    }
    ...
    if (-not $status.Ready -or
        ...
        [int]$status.ProcessId -ne $pid) {
        throw "Watchdog status is not stable Ready/LocalSystem/Session0 for PID $pid."
    }
