...
Assert-Contains -Text $manager -Pattern 'UnexpectedSetpointConfirmationSamples\s*=\s*2' -Description 'watchdog requires two matching unexpected post-restore setpoint samples before declaring ownership ambiguity'
Assert-Contains -Text $manager -Pattern 'unexpectedCandidate\s*==\s*after[\s\S]*unexpectedCandidateSamples\+\+[\s\S]*unexpectedCandidateSamples\s*>=\s*UnexpectedSetpointConfirmationSamples[\s\S]*OwnershipAmbiguous' -Description 'one anomalous 0x34/0x35 sample cannot prematurely terminate restore verification'
Assert-Contains -Text $manager -Pattern 'unexpectedCandidateSamples\s*=\s*0[\s\S]*Task\.Delay\([\s\S]*FirmwareRestoreVerificationPollInterval' -Description 'ambiguous-setpoint confirmation remains observational and bounded by the existing poll loop'
