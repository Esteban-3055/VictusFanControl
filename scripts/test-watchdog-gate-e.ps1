...
try {
    ...
    if ($readyMarker -notmatch '^READY\|.+\|authority=Custom\|cpu=30\|gpu=30\|ack=backend-ec\+tachs\+watchdog-owned$') {
        throw "Gate E READY marker does not prove backend EC+tachs acknowledgement plus watchdog OWNED 30/30: $readyMarker"
    }
    ...
    if (-not (Test-Path $journalPath)) {
        throw 'Durable OWNED journal disappeared before the forced-kill boundary.'
    }
